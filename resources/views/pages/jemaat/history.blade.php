<x-layouts.app>
    <x-slot name="title">Riwayat Kegiatan - Gereja Protestan Maluku</x-slot>

    <section class="bg-slate-50 py-24">
        <div class="mx-auto max-w-7xl space-y-8 px-4 sm:px-6 lg:px-8">
            <div class="space-y-3">
                <p class="text-sm font-semibold tracking-[0.18em] text-secondary-600 uppercase">Area Jemaat</p>
                <h1 class="font-serif text-4xl text-primary-900">Riwayat Kegiatan</h1>
                <p class="max-w-2xl text-base leading-7 text-slate-600">
                    Arsip kegiatan yang pernah Anda ikuti tersimpan di halaman ini.
                </p>
            </div>
            ...
            <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                @forelse ($kegiatan as $item)
                    <article class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                        <p class="text-sm font-medium text-slate-500">{{ $item->jenisKegiatan?->nama ?? 'Kegiatan Gereja' }}</p>
                ...
                @empty
                    <div class="rounded-[2rem] border border-dashed border-slate-300 bg-white px-6 py-12 text-center text-slate-500 md:col-span-2 xl:col-span-3">
                        Riwayat kegiatan Anda masih kosong.
                    </div>
                @endforelse
            </div>
